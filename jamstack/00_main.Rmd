...
bs_modal(
  ...
  title = "Using Markdown",
  body = includeMarkdown(system.file("markdown","modal.md",package="bsplus"))
  #body = includeMarkdown("~/Desktop/temp.md")
)
```
...
```{r, eval=FALSE}
mtc <- ggplot(mtcars, aes(mpg, wt)) + geom_point() + labs(title="GSP NAME", subtitle = "N-M wells fail at min threshold \n$3 - 4M estimated cost")
...
leaflet(gsa) %>% 
  ...
  addPolygons(data = gsa, weight = 1, opacity = 1, fillOpacity = 0.1, 
              ...
              popup = paste0("<em><b>", gsa$gsp_name, "</b></em><br>",
