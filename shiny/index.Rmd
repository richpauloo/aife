...
em$drawn$priority <- c("high","medium","low")
...
observe({
  ...
  temp <- filter(em$drawn, gsa == input$input_gsa)

  val <- switch(
    input$input_scenario,
    "specified groundwater decline" = 0,
    "minimum threshold" = temp$min_threshold,
    "minimum objective" = temp$min_objective
  )
...
observe({
  ...
  # Control the slider value
  updateRadioButtons(session, "input_scenario", choices = x, selected = "specified groundwater decline")
})
```
...
output$output_map <- renderLeaflet({
    ...
    icons <- awesomeIcons(
      ...
      iconColor = 'black',
      library = 'ion',
      markerColor = getColor(reactive_pts_color())
    )
    reactive_poly() %>% 
      leaflet() %>% 
      addTiles() %>% 
